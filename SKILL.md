...
- **Layout (transcript):** narrow chapter rail left + speaker-turn list right (one turn per row: speaker label + timestamp + text). Search bound to the turns corpus only. The chapter rail's link rows MUST use a flex layout (`display: flex; align-items: flex-start`) with `<time>` as `flex: 0 0 auto; min-width: 4.2rem` (HH:MM:SS fits) and the title wrapped in its own `<span class="ch-title">` set to `flex: 1 1 auto; min-width: 0`. Otherwise a wrapped chapter title falls *underneath* the timestamp column instead of indenting to align with the first line — a real-world bug invisible in any single-chapter example.
- **Briefing↔transcript visual parity (load-bearing — file-folder tabs depend on this).** The topbar layouts of the briefing and transcript views MUST render with the active tab in the **same X position** on screen. Folder tabs break the illusion the moment they slide between sibling views. Two rules that produce parity:
  1. **Identical brand text on both views.** Don't append " · Transcript" (or any view-name suffix) to the transcript's brand subline — the active folder tab already names the view. Adding a suffix changes brand width which shifts tabs.
  2. **Identical toggle slot on both views.** The hide-sidebar toggle must be present in the transcript topbar too, even though it hides a different sidebar (chapter rail on transcript, term inspector on briefing). Removing it on transcript shrinks the right-anchored cluster and lets the tabs drift left. Implementation: `body.inspector-hidden .chapter-rail { display: none }` (NOT inside a media query, per the v2.1.2 editorial rule), plus `body.inspector-hidden .transcript-shell { grid-template-columns: 1fr }` so the reading column widens when the rail hides.
- **Canonical examples:** `examples/podcast.html` (briefing) and `examples/podcast-transcript.html` (transcript) are the gallery references — both rendered from the same `tests/fixtures/episode.package.json` so the briefing↔transcript symmetry rules above are checkable, not just prose. `bin/render-podcast <package.json> [-o OUT]` is the skill-callable that produces the same pair from any podcastify `export-json` output.
- **Required primitives:**
  ...
  - **Theme: light by default, dark opt-in.** Do NOT auto-switch via `@media (prefers-color-scheme: dark)` — it surprises users on dark-OS setups who want the artifact to stay light. Instead: `body.dark { … }` carries the dark tokens, a small theme-toggle button in the topbar flips the class, the choice persists in `localStorage` (key `render-as-html.theme`). LocalStorage access must be try/catch'd for `file://` and sandboxed contexts. The button uses inline SVG moon/sun glyphs — Unicode ☾/☀ render as the curly-brace fallback character at small button sizes on iOS Safari (mobile audit 2026-05-24).
  - **Mobile (≤480px iPhone) topbar contract.** The topbar collapses to two rows — brand (clamp to 2 lines + ellipsis) + tab row with the theme toggle bottom-aligned inline. The brand subtitle is dropped on phone (redundant with the episode hero meta). `--topbar-h: auto` lets the topbar fit its content so the H1 below it stays visible on scroll. `#inspector-toggle` is hidden at ≤820px on both views (briefing extends to ≤1180 — see below).
  - **Inspector toggle visibility.** Hide `#inspector-toggle` at the breakpoints where its target column is no longer a collapsible sibling: briefing hides at ≤1180px (inspector column only mounts at ≥1181), transcript hides at ≤820px (chapter rail is sticky-collapsible at ≥821). Shared ≤820 rule lives in the renderer's COMMON_OVERRIDES; the briefing-only ≤1180 extension lives in the briefing's own overrides.
  - **A11y contract.** The folder-tabs nav is plain `<nav>` + `<a>` links with `aria-current="page"` on the active link — NOT `role="tablist"` / `role="tab"` (those describe in-page tab/tabpanel pairs and fail axe's `aria-required-children`). All interactive elements (tab links, topbar buttons, body links) must have a visible `:focus-visible` outline (WCAG 2.4.7). The transcript view MUST include a `<h1 class="sr-only">` inside `<main>` so the heading hierarchy doesn't start at `<h2>`. Inactive folder-tab text uses `--ink-soft` (not `--muted`) to clear WCAG AA 4.5:1 against `--paper-tint`.
