...
- **Hide-inspector toggle (reading-focus mode):** the inspector rail is useful for navigation and entity lookup, but distracting when the reader wants to actually read the prose. Every editorial artifact ships with a "hide inspector" button in the search toolbar that drops the right column entirely; the wide-screen grid reflows to two columns (`18rem 1fr`) and the prose stays at its 50rem measure. Pattern: a `body.inspector-hidden` class on the document (NOT a container class — it's a global reading mode); two `<span>`s inside the button (`.il-hide` shown by default, `.il-show` shown when hidden) so the label flips between "hide inspector" / "show inspector"; `aria-pressed` updated on click. Per-session only — do NOT persist via `localStorage` (the canonical implementation deliberately doesn't, so the inspector is back by default on the next visit). Reference implementation: `examples/editorial.html` lines 649–667 (CSS) + 985–988 (button) + 1788–1793 (handler).
