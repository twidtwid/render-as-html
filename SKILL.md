...
- **Required primitives:**
  ...
  - Inspector show/hide ("focus mode"): a toggle in the top bar that drops the inspector's grid column so the reading column reclaims the space; button label and `aria-pressed` flip with state; the toggle lives in the always-visible top bar so it stays reachable while the inspector is hidden.
...
- **HTML-native ≥3:** in-text `<mark>` search **with prev/next nav**; click-entity-to-open / cross-highlight; per-section copy-as-prompt; inspector show/hide focus mode; section scroll-spy if a section rail is present.
...
- **Hide-inspector toggle (reading-focus mode):** the inspector rail is useful for navigation and entity lookup, but distracting when the reader wants to actually read the prose. Every editorial artifact ships with a "hide sidebar" button in a dedicated topbar element (a thin `<header class="topbar">` element above `<div class="page">`, right-aligned via flex) that drops the right column entirely; the wide-screen grid reflows to two columns (`18rem 1fr`) and the prose stays at its 50rem measure. Pattern: a `body.inspector-hidden` class on the document (NOT a container class — it's a global reading mode); two `<span>`s inside the button (`.il-hide` shown by default, `.il-show` shown when hidden) so the label flips between "hide sidebar" / "show sidebar"; `aria-pressed` updated on click. The `body.inspector-hidden .rail-right { display: none }` rule MUST live OUTSIDE any wide-screen media query — otherwise the toggle silently does nothing on narrow viewports where the rail is in-flow rather than a grid column. Per-session only — do NOT persist via `localStorage` (the canonical implementation deliberately doesn't, so the inspector is back by default on the next visit). Reference implementation: `examples/editorial.html` lines 649–667 (CSS) + 985–988 (button) + 1788–1793 (handler).
