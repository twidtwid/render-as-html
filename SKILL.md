...
- **TODO** — `examples/podcast-transcript.html` is missing from the gallery. The transcript view is described in prose here but has no canonical reference file. Adding it would lock the briefing↔transcript symmetry rules above into checkable form and surface layout drift in the gallery rather than in downstream renders.
- **Required primitives:**
