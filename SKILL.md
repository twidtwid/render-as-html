---
name: render-as-html
description: Render a markdown file or piped content as a designed Apple-style HTML artifact. Use when the user says "render this as html", "make me a pretty version", "I want to read this carefully", "make it readable", "render <file>", or "/render-as-html". Output is designed for re-reading and sharing, not literal markdown→html conversion. Pairs with markdown source (which stays canonical).
---
...
| Input | Handling |
|---|---|
| MD file path | Read with `Read` tool |
| Piped content (in conversation) | Use the content directly |
| URL | Out of scope — say so |
...
| **Code** | Highlighted snippets (highlight.js via CDN) |
...
| **Images** | Embedded figures via `<img>` |
...
- Design tuner → sliders for accent color, font-size → `"Update --accent to hsl(…) in <file>"`
- Plan editor → drag-reorder workstreams → `"Re-prioritize plan.md to this order: 1) X, 2) Y, 3) Z"`
- Triage editor → bucket items into Now/Next/Later/Cut → `"Re-bucket these items in <source>: Now=[…] …"`
...
function copyPrompt() {
  ...
  const prompt = `In ${SOURCE_PATH}, apply these changes:\n${formatAsInstructions(changes)}`;
...
The prompt should:
- **Name the source file** (so Claude knows what to edit)
...
- **MD-impossible ≥3:** live filter, clickable cross-highlights, inline charts, toggles, sortable headers, copy buttons
...
- **Required primitives:** TL;DR block, sticky TOC sidebar with scroll-spy, footnote pattern, inline dense tables, callouts (note/warn), per-section "copy as markdown" button (hover-revealed on each h2)
- **Density:** 16px body, 1.55 line-height
- **MD-impossible ≥3:** TOC scroll-spy, per-section copy buttons, collapsible appendix, click-to-expand footnotes
...
- **MD-impossible ≥3:** live text search across events with highlight, date-range filter pills, cluster-collapse (month/year → count), tag-chip filter, per-event "copy as quote"
...
- **MD-impossible ≥3:** per-code-block copy button, live progress tracking, "stuck" prompt generator, conditional step visibility
- **Avoid:** plain numbered list with code blocks (that's markdown), making it look like `document`-shape (this is an *instrument*, not reading material)
...
- **MD-impossible ≥3:** live weight tuning recomputes winners, column sort by aggregate, "must-have" criterion toggle, "copy as recommendation" prompt
...
- **MD-impossible ≥3:** click-to-focus, hover-edge-highlight, search-to-focus, cluster toggle, shortest-path finder
...
- **Required primitives:** column headers with live count, draggable cards (HTML5 DnD, vanilla — no React-DnD), per-card one-line rationale text input, pre-sorted suggested distribution at load, sticky "copy as markdown" + "copy as prompt" export bar, undo
- **Density:** 14px body. Cards 80-120px tall. Columns 240-320px wide.
- **MD-impossible ≥3:** drag-between-columns, live column counts, copy-as-prompt exporting final assignments, undo, filter/search across all cards
...
- **Required primitives:** syntax-highlighted code (highlight.js via CDN), per-file diffs with `+`/`−` line gutters in green/red soft tints, inline margin annotations on specific lines, severity-coded finding cards, files-changed navigator, copy-link-to-finding buttons
- **Density:** 14px body, 13px code
- **MD-impossible ≥3:** syntax-highlighted code (impossible in MD without external renderer), severity-color findings, jump-to-file, click-to-copy individual findings, side-by-side before/after
...
- Body: Inter (via `https://rsms.me/inter/inter.css`) — well-designed, free
- Display: same Inter, heavier weight (700/800), tight letter-spacing (`-0.022em`)
...
- Single accent: warm coral `#e85d4f` / `#ff7066`. No corporate blue.
- Semantic: `ok=#4a9c5e`, `warn=#d98e2b`, `note=#4a90e2`
- High contrast — no gray-on-gray. WCAG AA minimum.
...
1. **Read the source** (file or piped content)
...
3. **Plan the instrument:**
   ...
   - What features would die in MD? Pick at least 3 — write them down before writing HTML.
   - Where does a diagram, chart, or spatial layout add information?
   - Which of the 8 dimensions am I using? (Aim for ≥4)
4. **Write the HTML** — single self-contained file, all CSS inline, all JS (vanilla, ~100-200 lines) inline
5. **Validate against the bar:** name the MD-impossible features. If <3, redesign before saving.
6. **Write to `~/Reports/<YYYY-MM-DD>-<slug>.html`**
7. **`open` the file** so it pops in browser
8. **Report back:** path, size, list of MD-impossible features
